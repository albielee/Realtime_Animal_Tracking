...
def draw_bbox(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if show_label:
            ...
            cv2.putText(image, class_names[class_ind] + " " + score_str, (x1, y1-4), cv2.FONT_HERSHEY_COMPLEX_SMALL,
...
#import sys
#sys.path.append("/content/Realtime_Animal_Tracking/Yolo/seq-nms-master/")
#sys.path.append("/content/build/temp.linux-x86_64-3.7/content/Realtime_Animal_Tracking/Yolo/seq-nms-master/")
...
def calculate_midpoints(boxes):
    midpoints = []
    for b in boxes:
        midpoints.append([b[2]-b[0], b[3]-b[1]])
    
    return np.array(midpoints)

def distance_to(p1,p2,radius):
    dist = m.sqrt(  ((p1[0]-p2[0])**2) +((p1[1]-p2[1])**2)  )
    print(dist < radius)
    return dist < radius
...
def findClosestCentroids(ic, X):
    ...
    for i in X:
        distance=[]
        for j in ic:
            distance.append(distance_to(i, j))
        assigned_centroid.append(np.argmin(distance))
    return assigned_centroid

def calculate_vectors(midpoints, prev_point_vectors):
    #if no previous midpoints then return a list of zeros of length midpoints
    if(len(prev_point_vectors) == 0):
        print("generating fake points")
        fake_unit_vectors = [[0,0]]*len(midpoints)
        return list(zip(midpoints, fake_unit_vectors))  

    #get_centroids = findClosestCentroids(centroids, X)

    #pm = previous midpoints
    #pm = np.array([i[0] for i in prev_point_vectors])
    ppv = prev_point_vectors
    #print(pm)
    #get local midpoints within radius
    r = 400
    mean_unit_vectors = []
    for mid in midpoints:
        local_points = np.array([p for p in ppv if distance_to(mid,p[0],r) is True])
        #print(local_points)
        #print(mid)
        unit_vectors = np.array([(mid-p)/m.sqrt((mid[0]-p[0])**2 + (mid[1]-p[1])**2) for p in local_points])
        #print([(mid-p)/m.sqrt((mid[0]-pm[0])**2 + (mid[1]-pm[1])**2) for p in local_points])
        if(len(unit_vectors) > 0):
          mean_unit_vectors.append(   [mid,sum(unit_vectors)/len(unit_vectors)] )
          print(mid)
          print(sum(unit_vectors)/len(unit_vectors))
          print(mean_unit_vectors)

    return mean_unit_vectors
...
def detect_video(Yolo, video_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    while True:
        ...
        bboxes = postprocess_boxes(pred_bbox, original_image, input_size, score_threshold)
        b = np.array([[box[0],box[1],box[2],box[3]] for box in bboxes])
        ...
        #b = np.array(list(map(list, zip(bboxes[:,0],bboxes[:,1],bboxes[:,2],bboxes[:,3]))))
        #[[x,y],[x,y]]
        midpoints = calculate_midpoints(b)
        unit_vectors = calculate_vectors(midpoints, prev_point_vectors)
        prev_point_vectors = unit_vectors
        ...
        fps_each_frame.append(fps2)
        # CreateXMLfile("XML_Detections", str(int(time.time())), original_image, bboxes, read_class_names(CLASSES))
        arrs = 100 # arrow size
        for p in prev_point_vectors:
            image = cv2.arrowedLine(image, (int(p[0][0]),int(p[0][1])), (int(p[0][0]+p[1][0]*arrs), int(p[0][1]+p[1][1]*arrs)), (255,0,0), 5)
...
def detect_video(Yolo, video_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    while True:
        ...
        try:
          ...
          for oi in org_ims:
            image = draw_bbox(oi, bboxes, CLASSES=CLASSES, rectangle_colors=rectangle_colors)
        ...
        #print("xyxy:",pred_coor)
        #print("conf:", pred_conf)
        #print("prop:", pred_prob)
        ...
        #  boxes.append([bboxes[i,0],bboxes[i,1],bboxes[i,2],bboxes[i,3]])
        ...
        s = np.array([bboxes[:,4]])
        scores.append(s)
        #print("classes ",classes_in_img)
        #print("boxes ", boxes)
        #print("scores", scores)
        ###seq_nms(boxes, scores, labels=classes_in_img, linkage_threshold=0.5, nms_threshold=0.3, score_metric='avg')
        ##image = draw_bbox(original_image, bboxes, CLASSES=CLASSES, rectangle_colors=rectangle_colors)
...
def calculate_vectors(midpoints, prev_point_vectors):
    ...
    mean_unit_vectors = []
    for mid in midpoints:
        local_points = np.array([p for p in pm if distance_to(mid,p,r) is True])
        unit_vectors = np.array([(mid-p)/m.sqrt((mid[0]-pm[0])**2 + (mid[1]-pm[1])**2) for p in local_points])
        print(mid)
        print(unit_vectors)
        if(len(unit_vectors) > 0):
            mean_unit_vectors.append(   [[mid],[sum(unit_vectors)/len(unit_vectors)]] )
...
def detect_video(Yolo, video_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    while True:
        ...
        prev_point_vectors = list(zip(midpoints,unit_vectors))
...
def detect_video(Yolo, video_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    while True:
        ...
        try:
            ...
            #boxes = tf.ragged.constant(boxes, dtype=np.array)
            #scores = tf.ragged.constant(scores, dtype=np.array)
            #classes = tf.ragged.constant(classes, dtype=np.array)
            #print(boxes.shape)
            #boxes = np.reshape(boxes, (frame_num, 10647, 4))
            #print(boxes)
            #tensor_boxes = tf.convert_to_tensor(boxes, dtype=tf.float32)
            #tensor_scores = tf.convert_to_tensor(scores, dtype=tf.float32)
            #tensor_classes = tf.convert_to_tensor(classes, dtype=tf.float32)
            seq_nms(boxes, scores, labels=classes, linkage_threshold=0.2, nms_threshold=0.3, score_metric='avg')
            #print(boxes.shape)
    
            for i, oi in enumerate(org_ims):

                image = draw_bbox(oi, bbox_list[i], CLASSES=CLASSES, rectangle_colors=rectangle_colors)
                image = cv2.putText(image, "Time: {:.1f}FPS".format(fps_each_frame[i], (0, 30), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 2))
        ...
        bboxes = postprocess_boxes(pred_bbox, original_image, input_size, score_threshold)
        print(len(bboxes))
        bbox_list  = np.append(bbox_list , bbox_list)
        #bboxes = nms(bboxes, iou_threshold, method='nms')
        c = bboxes[:, 5]
        classes.append(c)
        #boxes = []
        ...
        b = list(map(list, zip(bboxes[:,0],bboxes[:,1],bboxes[:,2],bboxes[:,3])))
        #print(b)
        #print(boxes)
        boxes.append(b)
